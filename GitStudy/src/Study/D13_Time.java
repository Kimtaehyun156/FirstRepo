...
public class D13_Time {
	 ...
	 * 		# java.time  패키지
	 * 		
	 * 		- Calendar의 인스턴스를 "set()을 통해 변화시킨다는 단점"을 보완한 클래스 
	 * 		- java.time 패키지의 클래스들은 시간을 계산한뒤 새로운 인스턴스를 반환한다
	 * 		  원본 인스턴스는 그대로 유지한다 
	 * 
	 * 		- LocalDate(날짜) , LocalTime(시간) , LocalDateTime(날짜,시간)이 있다 
	 */
	...
	public static void main(String[] args) {
		...
		//LocalDateTime : 날짜 / 시간 사용하기 위한 클래스 (T로 딜리미터가 되어있다) 짤라서 쓰세요..  
		...
		// 3. 시간을 변경하기 
		// - Calendar는 원본을 훼손하는 안좋은 설계였기 떄문에 여기서 차이가 난다 
		...
		// - 다양한 static 메서드와 ㅍ준들ㅇ르 이용해 문자열 형식을 지정할수 있다 
		// of.pattern : 패턴 직접 지정하기 
		...
		System.out.println(my_promise.format(DateTimeFormatter.BASIC_ISO_DATE));
