...
public class D05_Poker {
	 ...
	 * 
	 * 4. 풀하우스(Full House)트리플과 함께 하나의 페어를 가지고 있으면 메이드같은 랭킹일 경우 트리플의 숫자가 높은 플레이어가 승리
	 * 
	 * 5. 플러쉬(Flush)같은 무늬의 카드를 5장 가지고 있으면 메이드같은 랭킹일 경우 가장 높은 숫자를 보유한 플레이어가 승리
	 * 
	 * 6. 스트레이트(Straight)연속된 5장의 카드를 가지고 있으면 메이드A(Ace)의 경우는 10,J,Q,K,A와 A,2,3,4,5가
